...
- 클라우드 전환
  - 프론트엔드 전환하기
    ...
    - 목표
      ...
      - 프론트엔드를 Next.js로 전환 (JSP의 서버사이드 렌더링 이점을 SSR로 이어받음)
    ...
    - 전환 전략
      - 점진적인 페이지 단위 전환은 어려움 — 도메인이 클라우드 환경에 있지 않아 경로 기반 라우팅 구성이 불가능
        - 완화 방안: 도메인을 CloudFront로 향하게 하고 기존 온프레미스 서버를 커스텀 오리진으로 등록, 경로(path)별로 신규 경로만 ALB(Next.js ECS)/API Gateway로 분기 — 도메인 완전 이관 전에도 페이지 단위 전환을 1.5단계로 앞당길 수 있음 (자세한 내용은 [`target-architecture.md`](target-architecture.md#점진적-전환을-앞당기는-방법-cdn-리버스-프록시))
